...
internal class ScoreReport
{
    ...
    private void PrintBasicStats()
    {
        ...
        $"Min: {_scores.Min()} \n" +
        $"Max: {_scores.Max()} \n" +
        $"Average: {_scores.Average().ToString("0.0", CultureInfo.InvariantCulture)}");
    }
    ...
    {
        int[] sorted = _scores.OrderBy(score => score).ToArray();
    ...
    {
        int[] top = _scores.OrderByDescending(score => score).Take(topCount).ToArray();
    ...
    {
        int[] passingScores = _scores.Where(score => score >= Threshold).OrderByDescending(score => score).ToArray();
    ...
    {
        int[] failingScores = _scores.Where(score => score < Threshold).OrderByDescending(score => score).ToArray();
