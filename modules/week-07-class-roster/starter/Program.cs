...
public class Program
{
        ...
        while (choice != 4)
        {
            ...
            switch (choice)
            {
                case 1:
                    ...
                    string[] newNames = new string[ReadIntInRange($"How many students do you want to add? (1-{remainingSlots}): ", 1, remainingSlots)];
                    int[] newCredits = new int[newNames.Length];
                    ...
                    {
                        rosterNames[j] = newNames[j];
                        rosterCredits[j] = newCredits[j];
                    }
                ...
                case 2:
                    ...
                    Console.WriteLine("=== Class Roster ===");
                    string[] printOut = BuildRosterLines(rosterNames, rosterCredits, count);

                    foreach (string line in printOut)
                    {
                ...
                case 3:
                    ...
                    int miniChoice = ReadIntInRange("Sort by: ", 1, 2);

                    CopyUsedRoster(rosterNames, rosterCredits, count, out string[] sortNames, out int[] sortCredits);

                    switch (miniChoice)
                    {
                        case 1:
                            Array.Sort(sortNames, sortCredits, StringComparer.OrdinalIgnoreCase);
                            break;
                        case 2:
                            Array.Sort(sortCredits, sortNames);
                            break;
                    }
                    ...
                    {
                        Console.WriteLine($"{sortNames[i]}: {sortCredits[i]}");
                    }
                    Console.WriteLine();
                    break;
            }
    ...
    private static string[] BuildRosterLines(string[] names, int[] credits, int count)
    {
        ...
        {
            print[i] = $"{names[i]}: {credits[i]}";
        }

        return print; // Remove this placeholder and replace with the variable to created
    }
